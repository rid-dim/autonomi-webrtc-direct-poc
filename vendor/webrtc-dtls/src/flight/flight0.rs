            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        ...
                        //
                        // Upstream took `elliptic_curves[0]` unconditionally,
                        // i.e. whatever the client listed first, with no check
                        // that this implementation actually supports it.
                        //
                        // Chrome enables post-quantum key agreement by default
                        // and now lists X25519MLKEM768 (0x11ec) first. That maps
                        // to `NamedCurve::Unsupported`, and the handshake dies
                        // later with "invalid named curve" — so no current
                        // Chrome could complete a DTLS handshake at all.
                        //
                        // Pick the client's most-preferred curve that we can
                        // actually do, which is what curve negotiation means.
                        let selected = e
                            .elliptic_curves
                            .iter()
                            .copied()
